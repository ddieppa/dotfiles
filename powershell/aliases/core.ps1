...
function Show-CustomAliases {
    ...
    foreach ($file in $aliasFiles) {
        ...
        foreach ($m in $functionMatches) {
            ...
            if ($functionName -and $functionName -ne 'Set-SafeAlias' -and $functionName -ne 'Show-CustomAliases' -and $functionName -ne 'Reset-CustomAliases' -and $functionName -ne 'Get-AliasConflicts') {
